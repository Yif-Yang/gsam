...
class CelebAHQEdgeDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        for i in range(len(lines)):
            ...
            label_paths.append(
                os.path.join(opt.dataroot, 'CelebAMask-HQ-mask-anno', subfolder, lines[i].strip().zfill(5) + '_{}.png'))
    ...
    def get_label_tensor(self, path):
        ...
        label_tensor = torch.cat((tensor_edges_all, tensor_edges, torch.zeros_like(tensor_edges)), dim=0)
...
if __name__ == '__main__':
    ...
    # opt = CelebAHQEdgeDataset.modify_commandline_options(parser, is_train=True)
    from omegaconf import OmegaConf
    opt = OmegaConf.load(os.path.join(instruct_root,'configs/celebahqedge.yaml'))
    celebhq_dataset = CelebAHQEdgeDataset()
    opt.isTrain = False
    opt.phase = 'test'
