...
class DeepFashionDataset(Pix2pixDataset):
    ...
    def get_ref_video_like(self, opt):
        pair_path = os.path.join(instruct_root, './data/deepfashion_self_pair.txt')
        ...
        with open(os.path.join(instruct_root, ref_name)) as fd:
            ref = fd.readlines()
        ...
        for i in range(len(ref)):
            ...
            if items[0] in key_name.keys():
                ...
                ref_dict[items[0].replace('\\', '/')] = [it.replace('\\', '/') for it in key_name[items[0]]] + [
                    it.split(split)[-1] for it in random.sample(self.image_paths, min(len(self.image_paths), 20))]
            else:
                ref_dict[items[0].replace('\\', '/')] = [items[-1].replace('\\', '/')] + [it.split(split)[-1] for it in
                                                                                          random.sample(
                                                                                              self.image_paths,
                                                                                              min(len(self.image_paths),
                                                                                                  20))]
        train_test_folder = ('', '')
    ...
    def get_ref_vgg(self, opt):
        ...
        for i in range(len(lines)):
            ...
            if opt.phase == 'test':
                ...
                val = [items[-1].replace('\\', '/'),
                       random.choice(self.image_paths).split('DeepFashion/')[-1].replace('\\', '/')]
            ref_dict[key] = val
        train_test_folder = ('', '')
        return ref_dict, train_test_folder
    ...
    def get_label_tensor(self, path):
        ...
        candidate[:,0] -= int(img.shape[1]//8) ## Note that we hard-code this for no reason
        ...
        pose = Image.fromarray(cv2.cvtColor(canvas, cv2.COLOR_BGR2RGB)).resize((self.opt.load_size, self.opt.load_size),
...
if __name__ == '__main__':
    ...
    opt = OmegaConf.load('configs/deepfashion.yaml')
    deep_fashion_dataset = DeepFashionDataset()
    opt.isTrain = False
    opt.phase = 'test'
    # opt.isTrain = True
    # opt.phase = 'train'
    deep_fashion_dataset.initialize(opt, tokenizer=None, args=None)
    data_i = deep_fashion_dataset[0]
    print('dataset length: ', len(deep_fashion_dataset))
