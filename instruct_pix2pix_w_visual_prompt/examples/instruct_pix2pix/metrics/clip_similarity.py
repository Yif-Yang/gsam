...
class ClipSimilarity(nn.Module):
    ...
    # in case of visual prompt, the ground truth image is actually not involved in the evaluation process
    def forward(
        self, image_0: torch.Tensor, image_1: torch.Tensor, ref_image_0: torch.Tensor, ref_image_1: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        image_features_0 = self.encode_image(image_0)
        image_features_1 = self.encode_image(image_1)
        ref_image_features_0 = self.encode_image(ref_image_0)
        ref_image_features_1 = self.encode_image(ref_image_1)
        sim_0 = F.cosine_similarity(image_features_0, ref_image_features_0)
        sim_1 = F.cosine_similarity(image_features_1, ref_image_features_1)
        sim_direction = F.cosine_similarity(image_features_1 - image_features_0, ref_image_features_1 - ref_image_features_0)
        sim_image = F.cosine_similarity(image_features_0, image_features_1)
        return sim_0, sim_1, sim_direction, sim_image
